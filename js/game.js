...
const Game = (() => {
  ...
  function start(cfg) {
    ...
    let players;
    if (cfg.online) {
      // online: nomes fixos por assento para as mensagens baterem nos dois aparelhos
      players = [0, 1].map(i =>
        i === cfg.online.seat
          ? { name: `Jogador ${i + 1}`, type: 'human', level: null, group: null }
          : { name: `Jogador ${i + 1}`, type: 'remote', level: null, group: null });
    } else {
      players = [
  ...
  function loop(t) {
    ...
    if (state === 'shooting') {
      ...
      if (!PHYS.anyMoving(balls)) {
        if (match.mode === '9ball') resolve9(); else resolve8();
      }
    } else if (state === 'watch') {
  ...
  function doShoot(speed) {
    ...
    const c = cue();
    const v = Math.max(150, Math.min(PHYS.MAX_V, speed));
    c.vx = Math.cos(aimAngle) * v;
    c.vy = Math.sin(aimAngle) * v;
    c.spinX = spin.x;
    c.spinY = spin.y;
    strike = { t: 0, x: c.x, y: c.y, angle: aimAngle, pull0: 16 + (v - 250) / (PHYS.MAX_V - 250) * 30 };
    trail = [];
  ...
  function humanShoot() {
    ...
    doShoot(250 + (power / 100) * (PHYS.MAX_V - 250));
  }
  ...
  function finishTurn(foul, again, takeCue, castigo) {
    if (foul) {
      ...
      msg += nextState === 'ballInHand'
        ? `${match.players[current].name} com bola na mão`
  ...
  function netShot(m) {
    ...
    aimAngle = m.a;
    strike = { t: 0, x: m.cx, y: m.cy, angle: m.a, pull0: 16 + (m.p - 250) / (PHYS.MAX_V - 250) * 30 };
    trail = [];
    netAuth = false;
    state = 'watch';
  ...
  function setGhost(p) {
    const x = Math.max(R + 1, Math.min(W - R - 1, p.x));
  ...
  function getHud() {
    ...
    return {
      ...
      players: match.players.map(p => ({
        ...
        info: match.mode === '9ball' ? '' : openTable ? 'Mesa aberta' : remaining(p.group) === 0 ? `Na bola ${moneyBall()}!` : '',
      })),
    };
  }
  ...
  return {
    ...
    // ---- online ----
    netShot, netSnap, netSync, netPlace, netAim, netEnd,
    isNetAuth: () => !!(match && match.online && netAuth),
    makeRack: () => rack('8ball').map(b => ({ n: b.n, x: b.x, y: b.y })),
